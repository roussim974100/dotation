...
async function initRestitutionPage() {
  try {
    ...
    if (phase1EditLink) phase1EditLink.href = `restitution-phase1.html?id=${encodeURIComponent(id)}`;
    const phase1SummaryMeta = document.getElementById("phase1SummaryMeta");
    const phase1SummaryDates = document.getElementById("phase1SummaryDates");
    const phase1Summary = document.getElementById("phase1Summary");
    if (phase1Summary) phase1Summary.classList.toggle("d-none", !currentRestitution?.phase1ValidatedAt);
    if (phase1SummaryMeta && currentRestitution?.phase1ValidatedAt) {
      const d = new Date(currentRestitution.phase1ValidatedAt).toLocaleDateString("fr-FR", { day: "2-digit", month: "2-digit", year: "numeric", hour: "2-digit", minute: "2-digit" });
      phase1SummaryMeta.textContent = `Validée le ${d} par ${currentRestitution.phase1ValidatedBy || "inconnu"}`;
    }
    if (phase1SummaryDates) {
      const parts = [];
      if (currentRestitution.missionEndAt) parts.push(`Fin de mission : ${currentRestitution.missionEndAt}`);
      if (currentRestitution.returnedAt) parts.push(`Remise matériel : ${currentRestitution.returnedAt}`);
      phase1SummaryDates.textContent = parts.join(" · ");
    }
    ...
    function buildRestitutionPayload(forceDeferredSignature = false) {
      const returnedAt = currentRestitution?.returnedAt || "";
