...
let currentReadOnlyState = false;
...
function applyLockState(locked) {
  ...
  form.querySelectorAll("input, select, textarea, button").forEach((element) => {
    ...
    element.disabled = locked || currentReadOnlyState;
  });
...
function updateSignaturePreview(dataUrl) {
  ...
  previewWrap.classList.toggle("d-none", !hasSignature);
  previewImage.src = hasSignature ? dataUrl : "";
}

function applyReadOnlyState(readOnly) {
  currentReadOnlyState = readOnly;
  ...
  form.querySelectorAll("input, select, textarea, button").forEach((element) => {
    ...
    if (element.id === "saveDraftBtn" || element.id === "clearSignatureBtn") {
      element.disabled = readOnly || currentLockState;
      return;
    }
    element.disabled = readOnly || currentLockState;
  });

  if (readOnly && resumeHint) {
    resumeHint.textContent = "Votre profil est en lecture seule : vous pouvez consulter ce dossier, sans le modifier.";
  }
}
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  try {
    ...
    const canCreate = Boolean(session?.permissions?.includes("*") || session?.permissions?.includes("forms.create"));
    const canEdit = Boolean(session?.permissions?.includes("*") || session?.permissions?.includes("forms.edit"));
    if (!hasId && !canCreate) {
      alert("Votre profil ne permet pas de créer un nouveau dossier.");
      window.location.href = "index.html";
      return;
    }
    await loadDraftFromUrl(signaturePad);
    applyReadOnlyState(hasId && !canEdit);
    if (!hasId) {
      requestAnimationFrame(() => {
        requestAnimationFrame(() => hidePageLoader());
      });
